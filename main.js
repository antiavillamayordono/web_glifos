...
(function () {
  ...
  function initConcurso(){var mountC=$("[data-clasificacion]");if(!mountC)return;var cur=currentEdition();if(!cur){var tt=$("[data-concurso-title]");if(tt)tt.textContent="Aún no hay concursos";return;}
    ...
    var e=cur.e;var ey=$("[data-concurso-eyebrow]");if(ey)ey.textContent=(cur.active?t("js.contestLive"):t("js.contestLast"))+editionTitle(e);var st=$("[data-concurso-state]");if(st){st.textContent=cur.active?t("js.stateLive"):t("js.stateEnded");st.className="tag "+(cur.active?"tag--verano":"tag--navidad");}var pod=$("[data-podium]");if(pod)renderPodium(e,pod);renderClasificacion(e,mountC);var meta=$("[data-concurso-meta]");if(meta)meta.innerHTML='<span><b>'+e.glyphs.length+'</b> '+t("js.glyphs")+'</span><span><b>'+e.players+'</b> '+t("js.players")+'</span>';var search=$("[data-board-search]");if(search)search.addEventListener("input",function(){var q=norm(search.value);$$(".q tbody tr",mountC).forEach(function(r){var u=r.getAttribute("data-user")||"";var hit=q&&u.indexOf(q)!==-1;r.classList.toggle("is-hit",!!hit);r.classList.toggle("is-dim",q?u.indexOf(q)===-1:false);});});}
