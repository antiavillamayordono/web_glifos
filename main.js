...
(function () {
  ...
  function plu(n,sing,plur){return t(n===1?sing:plur);} // etiqueta singular/plural según el número
  ...
  /* ---------- controls ---------- */
  function isActiveEd(e){var ac=CFG.activeContest;return !!(ac&&e.year===ac.year&&e.category===ac.category);}
  function years(){var y={};DATA.forEach(function(e){if(!isActiveEd(e))y[e.year]=1;});return Object.keys(y).map(Number).sort(function(a,b){return b-a;});}
  function catsForYear(y){var c=[];DATA.forEach(function(e){if(e.year===y&&!isActiveEd(e))c.push(e.category);});return c.sort(function(a,b){return SRANK[a]-SRANK[b];});}
  ...
  /* ---------- PAGE: Concurso ---------- */
  function currentEdition(){var ac=CFG.activeContest;if(ac){var e=editionOf(ac.year,ac.category);if(e)return {e:e,active:true};return {active:true,pending:ac};}var s=editionsNewestFirst();return s.length?{e:s[0],active:false}:null;}
  function acTitle(ac){return SLBLf(seasonOf(ac.category))+" "+ac.year+" · "+CATL(ac.category);}
  function initConcurso(){var mountC=$("[data-clasificacion]");if(!mountC)return;var cur=currentEdition();if(!cur){var tt=$("[data-concurso-title]");if(tt)tt.textContent="Aún no hay concursos";return;}
    if(cur.pending){var ac=cur.pending,sea=seasonOf(ac.category);
      var eyp=$("[data-concurso-eyebrow]");if(eyp)eyp.textContent=t("js.contestLive")+acTitle(ac);
      var stp=$("[data-concurso-state]");if(stp){stp.textContent=t("js.stateLive");stp.className="tag "+(sea==="verano"?"tag--verano":"tag--navidad");}
      var podp=$("[data-podium]");if(podp)podp.innerHTML="";
      var metap=$("[data-concurso-meta]");if(metap)metap.innerHTML="";
      var bt=$(".board-tools");if(bt)bt.style.display="none";
      var lg=$(".legend");if(lg)lg.style.display="none";
      mountC.innerHTML='<p class="note">'+t("concurso.pending")+'</p>';return;}
    var e=cur.e;var ey=$("[data-concurso-eyebrow]");if(ey)ey.textContent=(cur.active?t("js.contestLive"):t("js.contestLast"))+editionTitle(e);var st=$("[data-concurso-state]");if(st){st.textContent=cur.active?t("js.stateLive"):t("js.stateEnded");st.className="tag "+(cur.active?"tag--verano":"tag--navidad");}var pod=$("[data-podium]");if(pod)renderPodium(e,pod);renderClasificacion(e,mountC);var meta=$("[data-concurso-meta]");if(meta)meta.innerHTML='<span><b>'+e.glyphs.length+'</b> '+plu(e.glyphs.length,"js.glyph","js.glyphs")+'</span><span><b>'+e.players+'</b> '+plu(e.players,"js.player","js.players")+'</span>';var search=$("[data-board-search]");if(search)search.addEventListener("input",function(){var q=norm(search.value);$$(".q tbody tr",mountC).forEach(function(r){var u=r.getAttribute("data-user")||"";var hit=q&&u.indexOf(q)!==-1;r.classList.toggle("is-hit",!!hit);r.classList.toggle("is-dim",q?u.indexOf(q)===-1:false);});});}

  /* ---------- PAGE: Histórico ---------- */
  function initHistorico(){var host=$("[data-historico]");if(!host)return;var ys=years();if(!ys.length)return;var state={year:ys[0],cat:catsForYear(ys[0])[0]};var meta=$("[data-edition-meta]"),mount=$("[data-hist-mount]"),search=$("[data-hist-search]");function applyFilter(){if(!search)return;var q=norm(search.value);$$(".q tbody tr",mount).forEach(function(r){var u=r.getAttribute("data-user")||"";r.classList.toggle("is-hit",!!(q&&u.indexOf(q)!==-1));r.classList.toggle("is-dim",q?u.indexOf(q)===-1:false);});}function render(){var e=editionOf(state.year,state.cat);if(!e){mount.innerHTML="";return;}if(meta)meta.innerHTML='<span><b>'+e.glyphs.length+'</b> '+plu(e.glyphs.length,"js.glyph","js.glyphs")+'</span><span><b>'+e.players+'</b> '+plu(e.players,"js.player","js.players")+'</span><span>'+editionTitle(e)+'</span>';renderClasificacion(e,mount);applyFilter();}if(search)search.addEventListener("input",applyFilter);buildControls(state,render);render();}
