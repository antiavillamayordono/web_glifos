...
(function () {
  ...
  function showGlyph(e,i){
    ...
    check.onclick=function(){if(!hasAns)return;var v=norm(input.value);if(!v)return;if(v===norm(g.name)){fb.textContent=t("js.correct");fb.className="fb ok";ans.textContent=t("js.solution")+g.name;ans.style.display="";}else{fb.textContent=t("js.wrong");fb.className="fb no";}};
  ...
  /* ---------- PAGE: Histórico ---------- */
  function initHistorico(){var host=$("[data-historico]");if(!host)return;var ys=years();if(!ys.length)return;var state={year:ys[0],cat:catsForYear(ys[0])[0]};var meta=$("[data-edition-meta]"),mount=$("[data-hist-mount]");function render(){var e=editionOf(state.year,state.cat);if(!e){mount.innerHTML="";return;}if(meta)meta.innerHTML='<span><b>'+e.glyphs.length+'</b> '+t("js.glyphs")+'</span><span><b>'+e.players+'</b> '+t("js.players")+'</span><span>'+editionTitle(e)+'</span>';renderClasificacion(e,mount);}buildControls(state,render);render();}

  /* ---------- Modo libre ---------- */
  function initRandom(){var host=$("[data-random]");if(!host)return;var pool=[];DATA.forEach(function(e){e.glyphs.forEach(function(g,i){pool.push({e:e,i:i,name:g.name});});});var cur=null;var title=$("[data-r-title]",host),stage=$("[data-r-stage]",host),input=$("[data-r-input]",host),fb=$("[data-r-fb]",host);function pick(){cur=pool[Math.floor(Math.random()*pool.length)];title.textContent=t("js.randTitle")+SLBLf(seasonOf(cur.e.category))+" "+cur.e.year;stage.innerHTML='<img src="'+glyphImg(cur.e,cur.i)+'" alt="Jeroglífico" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\';"><div class="daily__ph" style="display:none"><div class="q3">¿ ? ?</div><p>Imagen por añadir</p></div>';input.value="";fb.textContent="";fb.className="fb";}function check(){if(!cur){pick();return;}var v=norm(input.value);if(!v)return;if(v===norm(cur.name)){fb.textContent=t("js.randCorrect")+cur.name+t("js.emoji");fb.className="fb ok";}else{fb.textContent=t("js.randWrong");fb.className="fb no";}}$("[data-r-check]",host).addEventListener("click",check);$("[data-r-new]",host).addEventListener("click",pick);input.addEventListener("keydown",function(e){if(e.key==="Enter")check();});pick();}
